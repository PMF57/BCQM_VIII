...
- Community detection is on the UNDIRECTED projection of the chosen edge set (core or core+halo).
...
def load_run(p: Path, edge_source: str) -> Tuple[List[Tuple[int, int]], List[int]]:
    ...
    if edge_source == "all":
        ...
        if isinstance(halo_edges, list):
            edges.extend([(int(u), int(v)) for u, v in halo_edges])
...
def main() -> None:
    ...
    for p in files:
        ...
        if args.n_filter is not None and (math.isnan(n) or abs(n - args.n_filter) > 1e-12):
            continue
        edges, evs = load_run(p, args.edge_source)
