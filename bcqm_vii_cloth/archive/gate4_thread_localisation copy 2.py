# PROVENANCE: BCQM_VII Gate-4 analysis; add --tag output naming; v0.3; 2026-02-02
#!/usr/bin/env python3
...
def main():
    ...
    ap.add_argument("--tag", default=None, help="Optional tag for output filenames. If omitted, derived from run_dir name and options.")
    ...
    args = ap.parse_args()
# Derive an output tag if not provided: based on run_dir basename and key options.
run_name = Path(args.run_dir).name.rstrip("/").replace(" ", "_")
if args.tag is None or str(args.tag).strip() == "":
    args.tag = f"{run_name}__part-{args.partition_source}__sg-{args.supergraph_source}__res-{args.resolution}"
else:
    args.tag = str(args.tag).replace(" ", "_")
    ...
    df_stats.to_csv(out_dir / f"gate4_{args.tag}_stats_seedwise.csv", index=False)
    df_hops.to_csv(out_dir / f"gate4_{args.tag}_hopdist_seedwise.csv", index=False)

    print("Wrote:")
    print(" -", out_dir / f"gate4_{args.tag}_stats_seedwise.csv")
    print(" -", out_dir / f"gate4_{args.tag}_hopdist_seedwise.csv")
