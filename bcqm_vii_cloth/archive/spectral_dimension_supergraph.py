#!/usr/bin/env python3
# PROVENANCE: BCQM_VII Stage-2 analysis; Test 2.1 spectral dimension on super-graph; v0.1; 2026-02-03

"""Estimate spectral dimension d_s(t) on the community super-graph (core/core by default).
...
Pipeline per RUN_METRICS file:
  ...
  4) estimate P0(t) by Monte Carlo random walks:
       choose start node uniformly in GCC, run walk for t steps, count returns
  5) compute d_s(t) via finite differences in log space

Outputs:
  - <out_dir>/<tag>_spectral_dim_curves.csv   (seedwise P0(t) and d_s(t))
  - <out_dir>/<tag>_spectral_dim_summary.csv  (per (N,n): effective plateau estimate, with warnings)
...
def random_walk_return_probs(G: nx.Graph, t_max: int, n_walks: int, rng: np.random.Generator) -> np.ndarray:
    ...
    P = np.zeros(t_max+1, dtype=float)
    # define P0(0)=1
    P[0]=1.0
    # Monte Carlo
...
def plateau_estimate(ds: np.ndarray, t_min: int, t_max: int) -> Tuple[float,float,int]:
    """Return mean,std,count of ds in window where finite."""
    sl = ds[t_min:t_max+1]
...
def main():
    ...
    for f in files:
        ...
        P0 = random_walk_return_probs(GCC, args.t_max, args.n_walks, rng)
    ...
    if not df_runs.empty:
        summary = df_runs.groupby(["N","n","edge_source"]).agg(
            runs=("seed","count"),
            K_mean=("K_super","mean"),
