...
def run_single_v_glue(cfg: Dict[str, Any], N: int, n: float, seed: int) -> None:
    ...
    # Cloth summary (end-of-run): persistent concurrency hits across bins (core vs all).
    if cloth_cfg["enabled"]:
        min_hits = int(cloth_cfg["min_bin_hits"])
        edge_hits: Dict[tuple, int] = {}
        event_hits: Dict[int, int] = {}
        for rec in cloth_ledger:
            for u,v,c in rec.get("edges_core", []):
                edge_hits[(int(u), int(v))] = edge_hits.get((int(u), int(v)), 0) + 1
            for e,c in rec.get("events_core", []):
                event_hits[int(e)] = event_hits.get(int(e), 0) + 1
        core_edges = {k for k,v in edge_hits.items() if int(v) >= min_hits}
        core_events = {k for k,v in event_hits.items() if int(v) >= min_hits}
        edge_hits_all: Dict[tuple, int] = {}
        event_hits_all: Dict[int, int] = {}
        for rec in cloth_ledger:
            for u,v,c in rec.get("edges_all", []):
                edge_hits_all[(int(u), int(v))] = edge_hits_all.get((int(u), int(v)), 0) + 1
            for e,c in rec.get("events_all", []):
                event_hits_all[int(e)] = event_hits_all.get(int(e), 0) + 1
        halo_edges = {k for k,v in edge_hits_all.items() if int(v) >= min_hits and k not in core_edges}
        halo_events = {k for k,v in event_hits_all.items() if int(v) >= min_hits and k not in core_events}
